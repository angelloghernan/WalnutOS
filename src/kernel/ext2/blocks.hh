...
class Superblock {
  public:
    ...
    [[nodiscard]] auto cache_read(wlib::ahci::AHCIState *disk)
    ...
    // Whenever ext2 is not already present on the mounted disk, this should be
    // called in order to set up the disk for use with ext2. We set up the block
    // group descriptor table and the root directory accordingly.
    //
    // As you would expect, _all existing data on the disk is liable to being
    // overwritten_. As such, this should only be called under the assumption
    // that this disk is totally unformatted.
    [[nodiscard]] auto format_superblock()
        -> wlib::Result<wlib::Null, wlib::ahci::IOError>;
