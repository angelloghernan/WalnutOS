...
// Number of blocks per block group when this OS formats the hard drive for
// ext2.
auto static constexpr WNOS_BLOCK_GROUP_BLOCKS = 1953;

// Number of inodes per block group when this OS formats the hard drive for
// ext2.
...
// Where "normal" blocks really start, after the reserved boot block and the
// superblock
...
class Ext2FS {
  public:
    ...
    auto find_inode(INodeNum parent_dir, wlib::str const name)
        -> wlib::Result<INodeNum, IOError>;

    auto create_inode(INodeNum parent_dir, wlib::str const name)
        -> wlib::Result<INodeNum, IOError>;

    auto write_inode(INodeNum inode, wlib::Slice<u8> const &buffer)
        -> wlib::Result<u32, IOError>;

    auto read_inode(INodeNum inode, wlib::Slice<u8> &buffer)
        -> wlib::Result<u32, IOError>;

    auto read_block_descriptor(wlib::Slice<u8> &buffer)
        -> wlib::Result<wlib::Null, IOError>;
  ...
  private:
    ...
    auto read_block(u32 block_num, wlib::Slice<u8> &buffer)
        -> wlib::Result<wlib::Null, IOError>;
