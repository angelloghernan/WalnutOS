...
extern "C" void kernel_main() {
    ...
    for (i8 i = 1; i < 33; ++i) {
        auto num_blocks = 0;
        terminal.print_line("Expecting ", i8(NUM_BLOCKS / alloc::round_up_pow2(i)), " blocks");
        for (i8 j = 0; j < NUM_BLOCKS / (alloc::round_up_pow2(i)); ++j) {
            auto block = allocator.kalloc(PAGESIZE * i);
            assert(block.some(), "Allocator returned nothing");
            arr[num_blocks] = block.unwrap();
            terminal.print_line("Allocator returned: ", (void*)block.unwrap());
            ++num_blocks;
        }
