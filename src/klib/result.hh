...
template <typename T = Null, typename E = Null> class Result {
  public:
    ...
    requires concepts::is_different_type<T, E>
    {
        return Result(false, util::move(err));
    }

    auto constexpr static Err(T &&err) -> Result<T, E>
    requires concepts::is_same_type<T, E>
    {
        return Result(false, util::move(err));
    }
    ...
    auto constexpr static Ok() -> Result<T, E> { return Result(true); }

    auto constexpr static Err() -> Result<T, E> { return Result(false); }
    ...
    auto constexpr as_err() -> E & { return m_data.err; }

    auto constexpr as_err() const -> E const & { return m_data.err; }

    auto constexpr as_ok() -> T & { return m_data.data; }

    auto constexpr as_ok() const -> T const & { return m_data.data; }

    auto constexpr matches() const -> bool { return m_is_success; }

    auto constexpr is_err() const -> bool { return !m_is_success; }

    auto constexpr is_ok() const -> bool { return m_is_success; }

    void constexpr into_err() { m_is_success = false; }

    void constexpr into_ok() { m_is_success = true; }

    template <typename F> auto constexpr ok_or_else(F lambda) && -> T {
        if (is_ok()) {
            return m_data.data;
        } else {
            return lambda();
...
// clangd doesn't like "requires" statements for some reason
...
#else
    constexpr ~Result()
    requires(!type_traits::is_trivially_destructible<T>)
    {
        if (m_is_success) {
            m_data.data.~T();
        } else {
  ...
  private:
    ...
    constexpr Result(bool success, T &&obj)
        : m_data(util::move(obj)), m_is_success(success) {}

    constexpr Result(bool success, E &&obj)
    requires concepts::is_different_type<T, E>
        : m_data(util::move(obj)), m_is_success(success) {}
